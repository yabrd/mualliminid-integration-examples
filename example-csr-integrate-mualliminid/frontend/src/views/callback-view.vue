  ...
  <div class="min-h-screen flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950 transition-colors duration-200">
    <Card class="max-w-sm text-center p-6 pb-4 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 shadow-xl dark:shadow-none">
      <div v-if="status === 'processing'" class="space-y-4">
        ...
        <h2 class="text-xl font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wide">Menghubungkan Sesi...</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400">Sedang memverifikasi kode otorisasi dari server SSO MualliminID.</p>
      ...
      <div v-else-if="status === 'error'" class="space-y-4">
        ...
        <h2 class="text-xl font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wide">Autentikasi Gagal</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400">{{ errorMessage }}</p>
        <p class="text-xs text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-950/20 p-2 font-mono break-all">{{ errorDetails }}</p>
        <button @click="goToLogin" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 w-full block rounded-xl transition-all uppercase tracking-wider text-xs cursor-pointer">
          Kembali ke Login
        </button>
      </div>
    </Card>
...
const exchangeCode = async () => {
  ...
  if (!state || !savedState || state !== savedState) {
    ...
    errorDetails.value = 'Parameter state tidak valid atau kedaluwarsa. Silakan coba login kembali.';
    return;
  }
  ...
  try {
    ...
    if (!verifier) {
      throw new Error('PKCE verifier tidak ditemukan di sessionStorage. Silakan coba login kembali.');
    }
    ...
    try {
      ...
      throw new Error('Gagal memverifikasi akun lokal di backend: ' + backendErr.message);
    }
